            ...
            <Container maxWidth="xl" className='bg-black'>
                <Toolbar disableGutters>
                    ...
                    <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
                        ...
                        >
                            {!isDashboard ? pages.map((page) => (
                                <MenuItem key={page} onClick={handleCloseNavMenu}>
                                    <Link href={`/${page.toLowerCase()}`}>
                                        <Typography sx={{ textAlign: 'center', textDecoration: 'none' }}>{page}</Typography>
                                    </Link>
                                </MenuItem>
                            )) : (<div>
                                <MenuItem key={names} onClick={handleCloseNavMenu}>
                                    <Typography sx={{ textAlign: 'center', textDecoration: 'none' }}>{names}</Typography>
                                </MenuItem>
                                <MenuItem key={'logout'} onClick={handleCloseNavMenu}>
                                    <Link href={`/`}>
                                        <Typography sx={{ textAlign: 'center', textDecoration: 'none' }}>{'Logout'}</Typography>
                                    </Link>
                                </MenuItem>
                            </div>)}
                        </Menu>
                    ...
                    <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }} className='flex justify-end'>
                        {!isDashboard ? pages.map((page) => (
                            ...
                            >
                                <Link href={`/${page.toLowerCase()}`}>
                                    <Typography sx={{ textAlign: 'center', textTransform: 'capitalize' }} className='hover:underline'>
                                        {page}
                                    </Typography>
                                </Link>
                            </Button>
